...
<script>
	...
	export default {
		...
		methods: {
			...
			filterPoints(points) {
				let tempGps
				tempGps = gpsFilter.velocityFilter(points, 0, 10)
				tempGps = gpsFilter.accelerationFilter(tempGps, 0, 10)
				return tempGps
			},
			// 计算里程
			handleDistance(points){
				const gpsTool = new GpsTool()
				let distance = 0
				points.forEach((item, index) => {
					if(index === 0)return
					distance += gpsTool.distance(points[index-1].latitude, points[index-1].longitude, item.latitude, item.longitude)
				})
				return distance
			},
			// 处理定位信息
			handleLocation(e) {
				...
				console.log(realGps)
				if(this.tempGps.length > 0){
					let lastGps = this.tempGps[this.tempGps.length-1]
					if(lastGps.latitude === realGps.latitude && lastGps.longitude === realGps.longitude)return
				}
				// 判断当前gps跟上一个gps是否一样
				if(this.tempGps.length > 20){
					// 过滤后的点
					let tempFilterGps = this.filterPoints(this.tempGps)
					// 过滤后的里程
					let tempDistance = this.handleDistance(tempFilterGps)
					this.distance += tempDistance
					console.log(`${this.distance} - ${tempDistance}`)
					// 过滤后的放到真实的gps信息里
					this.filterGps.concat(this.tempGps)
					// 放入polyline[points
					tempFilterGps.forEach(item => {
